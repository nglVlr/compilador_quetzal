...
/**
 * Analizador Sintáctico (Parser) para Lenguaje Quetzal.
 *
 * Recibe la lista de tokens del Analizador Léxico y produce un AST.
 *
 * Gramática implementada (precedencia de menor a mayor):
 *
 *   programa       → sentencia* EOF
 *   sentencia      → importar | exportar | declaracion | sentenciaSimple
 *   declaracion    → declVariable | declLista | declJsn | declFuncion | declObjeto
 *   sentenciaSimple→ si | mientras | hacer | para | paraEn | intentar
 *                  | retornar | romper | continuar | lanzar
 *                  | expresionStmt | asignacion
 *
 *   expresion      → ternaria
 *   ternaria       → logicaO ('?' expresion ':' expresion)?
 *   logicaO        → logicaY ('o' | '||' logicaY)*
 *   logicaY        → igualdad ('y' | '&&' igualdad)*
 ...
 *   primario       → literal | ID | '(' expresion ')' | 'nuevo' ID '(' args ')'
 */
public class AnalizadorSintactico {
    ...
    public AnalizadorSintactico(List<Token> tokens) {
        // Filtrar NUEVA_LINEA para simplificar el parser (ya tenemos el léxico correcto)
    ...
    private Nodo parsearPostfijo() {
        ...
        while (true) {
            if (verifica(TipoToken.PUNTO)) {
                ...
                String campo = consumir(TipoToken.IDENTIFICADOR).getValor();
